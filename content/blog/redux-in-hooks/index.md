...
`useReducer` is an outstanding construct for managing state in React applications; however, there can be situations in which passing around a dispatch function can be less convenient than a typical callback function. This post will walk through ways of managing of managing this. We'll first look at wrapping dispatch actions of our reducer into stand-alone functions which can be called with just the arguments they need; if you've used Redux, this will be familiar. We'll then discuss some issues with this, and look at a safer, simpler approach, adding static typings to your reducer (and concomitant dispatch function).
...
This reducer manages things like desktop vs mobile views, login state, if a publicly-available user's book collection is being viewed, online/offline status, etc.
...
Everything above hinges on `getStatePacket` working, so let's turn there.
...
This function wraps our dispatch function so a function (or "thunk") can be dispatched. That's what allowed `requestDesktop` and `requestMobile` to work, above. If our new dispatch is passed an object, we dispatch it, and are done. If a function is dispatched, we _call_ the function, passing it both the original dispatch function, as well as a function that returns the current state (Redux called this `getState`).
...
After all that build-up, the end result is relatively straightforward.
...
We loop through each action creator, and create a new function that forwards any passed arguments to the original function, takes the result, and passes it to dispatch. If the result is itself a new function, ie a thunk, `getStatePacket` will call it, and pass in dispatch. If that result is just an object, then `getStatePacket` will just dispatch it and be done.

If `Object.entries` is scary, know that it's basically just `Object.keys`, except it _also_ gives you the corresponding values. If `reduce` is scary, know that that's normal. If you're not well familiar with Array.prototype.reduce, I'd highly recommend you read through [the docs](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/reduce), and practice with it. It's one of the most versatile, useful tools you can have in your toolbox.

### Warning

Did you catch how our action creators were re-created every time the reducer's state changed? This was essential to prevent stale closures; but it also means that it needs to be present in any dependency lists. For example, if you call one of these action creators in `useEffect` (which is very likely), then you'll need to be sure you add the action creator itself to the dependency list.

It's also worth noting that re-binding all of these functions on every state change will incur a performance cost. It's likely this cost will be small, but it's also likely this cost may become noticeable in hot paths, if this pattern is used a lot.

## Re-thinking our life choices

Let's step back for a moment and remember what our original goal was: to make dealing with our reducers easier. Most people, myself included, find it easier to work with an action creator, then a raw dispatch function, since they're easier to work with, essentially being statically typed for free, even if you occasionally have to give TypeScript's type inference a hand. So with action creators, our editor will tell us every action creator there is (via autocompletion when importing them), and it'll tell us which arguments they take, often with types inferred automatically (or added manually, when needed).

But can we get these same benefits without all the overhead we saw above? It turns out we can come pretty close. Let's take a look at adding types right to our reducer, so TypeScript will give us the same information when using the raw dispatch function, alone.

### Tweaking our payload

I'll start with a preliminary tip, which Dan Abramov was nice enough to share with me. In your reducer, you don't _have_ to dispatch an object; you're allowed to dispatch an array, if you want. This allows you to do things like <br><br> `dispatch(["ADD_TODO", { id: 1, title: "Write Blog" }])`<br><br>as opposed to<br><br>`dispatch({ type: "ADD_TODO", value: { id: 1, title: "Write Blog" }})`.

I've found this to be a nice idiom to work with, so let's try it out

### Our new reducer

To keep things simple, let's start with a fresh, smaller reducer. This one should suit our purposes well
...
It doesn't do anything, but that's ok; we're only interested in seeing how to add typings to it.
...
`actions` is a type, with the union of all possible action dispatches we'll need. If it seems like too much work to add a different type tuple for each possible switch branch, keep in mind that before we were adding a whole new **function** for each switch branch.
...
But you can always jump to the reducer definition to quickly peak at the type declaration, and regardless, if you try to use it wrong, TypeScript will tell you.
...
```javascript
const staggeredIncrementX = (dispatch, first, second) => {
...
```javascript
const staggeredIncrementX = (
  dispatch: (packet: actions) => any,
  first,
  second
) => {
  dispatch(["SET_X", first]);
