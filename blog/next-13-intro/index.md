...
Before we start, please note that RSC support in Next 13 is still in beta. The things I'll show you might change, and there are missing pieces which are works in progress (mutations in particular) so please don't go shipping any of this to production.

I want this post to be a crude, quick and dirty introduction, to hopefully spark some excitement at what's coming.

# React Server Components

Typically React components (when used in an SSR framework like Next) will render on the server, send the html down to your user's browser, along with scripts that load react, and re-render the whole app. This re-rendering is called hydration.
...
RSCs are tightly integrated with Suspense. You can place Suspense boundaries inside of your markup, and React will send down what's ready, along with the Suspense boundaries' loading placeholders. As the data for those Suspense boundaries come in, React will _update_ your page with the content for those Suspense boundaries. That's right, RSCs aren't just a one-time server render; they're interactive with the page, continuously updating segments as they come in.

# Let's get started

Let's dive in and build an _extremely_ contrived todo list prototype to see how data loading works. Nothing useful, not even any bit of nice formatting. Just the absolute minimum to show how nice these new data loading capabilities are. The RSC will server render some initial data, and the page will then be able to filter, and request new data from the RSC. That's right, RSCs are interactive. It's not just an initial render; the page and server actually stay in sync as the user interacts.

We'll just use static data here, rather than try to rig up a database. But we'll get the static data from a Next api endpoint, just to guarantee that we are in fact performing server-side network requests, and we'll manually slow it down so we can focus on loading states.
...
Let's start with our data endpoints. They're not very interesting, but they'll be good enough to show the nuts and bolts of data fetching. We have a todos endpoint
...
Let's turn to our page.tsx file; this is our RSC. Here's a naive first draft of our component.
