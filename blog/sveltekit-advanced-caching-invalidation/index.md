...
This will be a longer, more difficult post than most of what I usually write, since we're covering harder topics. This post will essentially show you how to implement common features of popular data utilities like react-query; but instead of pulling in an external library, we'll be using the web platform, and SvelteKit features only.
...
But speaking of React Query, it [was just released](https://tanstack.com/query/v4/docs/svelte/overview) for Svelte! So if you find yourself leaning on manual caching techniques _a lot_, be sure to check that project out, and see if it might help.
