...
Now let's add a simple form feature to our `/list` page:
...
Yep, forms can post directly to our normal page loaders. Now we can add a search term in the search box, hit <kbd>Enter</kbd>, and a "search" term will be appended to the URL's query string, which will re-run our loader and search our to-do items.
...
But _how_ do we actually update that cache busting value when we need to? Since it's stored in a cookie, iwe can call it like this from any server action:
